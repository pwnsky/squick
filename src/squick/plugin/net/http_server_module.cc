...
bool HttpServerModule::AddMsgCB(const std::string& strCommand, const HttpType eRequestType, const HTTP_RECEIVE_FUNCTOR_PTR& cb)
{
	...
	if (!it)
	{
		mMsgCBMap.AddElement(eRequestType, SQUICK_SHARE_PTR<std::map<std::string, HTTP_RECEIVE_FUNCTOR_PTR>>(SQUICK_NEW std::map<std::string, HTTP_RECEIVE_FUNCTOR_PTR>()));
	}
...
bool HttpServerModule::ResponseMsg(SQUICK_SHARE_PTR<HttpRequest> req, const std::string& msg, WebStatus code,
