...
class SceneInfo : public MapEx<int, SceneGroupInfo> {
  public:
...
class SceneModule : public ISceneModule {
  public:
    ...
    virtual int GetPropertyInt32(const int scene, const int group,
                                 const std::string &propertyName); // equal to (int)GetPropertyInt(...), to remove C4244 warning
  ...
  protected:
    bool SwitchScene(const Guid &self, const int nTargetSceneID, const int nTargetGroupID, const int type, const Vector3 v, const float fOrient,
                     const DataList &arg);
  ...
  protected:
    ...
    int OnPropertyEvent(const Guid &self, const std::string &propertyName, const SquickData &oldVar, const SquickData &newVar, const DataList &argVar,
                        const INT64 reason);
    int OnRecordEvent(const Guid &self, const std::string &recordName, const RECORD_EVENT_DATA &eventData, const SquickData &oldVar, const SquickData &newVar,
                      const DataList &argVar);
