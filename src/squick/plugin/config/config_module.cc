...
std::vector<std::string> ConfigModule::GetFieldList(const std::string & strStructName)
{
	...
	{
		std::string strStructName;
		for (SQUICK_SHARE_PTR<std::string> pData = pStructTypeData->mmStructAttribute.First(strStructName); pData != NULL; pData = pStructTypeData->mmStructAttribute.Next(strStructName))
		{
			xList.push_back(strStructName);
		}
	}
