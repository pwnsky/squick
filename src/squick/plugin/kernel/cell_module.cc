...
CellModule::CellModule(IPluginManager *p) { pPluginManager = p; }
...
bool CellModule::AfterStart() {
    m_pKernelModule->AddClassCallBack(excel::Player::ThisName(), this, &CellModule::OnObjectEvent);

    // NF SYNC
    bool bCell = false;
    std::shared_ptr<IClass> xServerLogicClass = m_pClassModule->GetElement(excel::Server::ThisName());
            ...
            for (; it != mtCellInfoMap.end(); it++)
            {
                    ...
                    for (int nPosX = vLeftBot.X(); nPosX < vRightTop.X(); nPosX += nCellWidth)
                    {
                            for (int nPosY = vLeftBot.Y(); nPosY < vRightTop.Y(); nPosY += nCellWidth)
                            {
                                    ...
                                    SQUICK_SHARE_PTR<SceneCellInfo> pCellInfo = SQUICK_SHARE_PTR<SceneCellInfo>(SQUICK_NEW SceneCellInfo(it->first,
...
const int CellModule::GetAroundCell(const int &sceneID, const int &groupID, const Guid &selfGrid, SQUICK_SHARE_PTR<SceneCellInfo> *gridList,
...
const int CellModule::GetAroundCell(SQUICK_SHARE_PTR<SceneCellInfo> pGridInfo, SQUICK_SHARE_PTR<SceneCellInfo> *gridList,
                                    ECELL_AROUND around /*= ECELL_AROUND_9 */) {
    int nObjectCount = 0;
...
int CellModule::OnObjectEvent(const Guid &self, const std::string &classNames, const CLASS_OBJECT_EVENT classEvent, const DataList &var) {
    if (CLASS_OBJECT_EVENT::COE_CREATE_FINISH == classEvent) {
        m_pKernelModule->AddPropertyCallBack(self, excel::IObject::Position(), this, &CellModule::OnPositionEvent);
    }
...
int CellModule::OnPositionEvent(const Guid &self, const std::string &propertyName, const SquickData &oldVar, const SquickData &newVar, const INT64 reason) {
    ...
    const int groupID = m_pKernelModule->GetPropertyInt32(self, excel::IObject::GroupID());
