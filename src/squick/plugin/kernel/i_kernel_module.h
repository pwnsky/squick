...
public:
    ...
    ////////////////event//////////////////////////////////////////////////////////
    template<typename BaseType>
    bool AddClassCallBack(const std::string& className, BaseType* pBase, int (BaseType::*handler)(const Guid&, const std::string&, const CLASS_OBJECT_EVENT, const DataList&))
    {
    ...
    virtual SQUICK_SHARE_PTR<IObject> CreateObject(const Guid& self, const int sceneID, const int groupID, const std::string& className, const std::string& configIndex, const DataList& arg) = 0;
	...
	inline int GetRecordInt32(const Guid& self, const std::string& recordName, const int row, const int col) { return (int)GetRecordInt(self, recordName, row, col); };
	...
	inline int GetRecordInt32(const Guid& self, const std::string& recordName, const int row, const std::string& colTag) { return (int)GetRecordInt(self, recordName, row, colTag); };
