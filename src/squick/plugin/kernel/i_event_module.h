...
public:
	...
	bool AddEventCallBack(const int eventID, BaseType* pBase, int (BaseType::*handler)(const int, const DataList&))
	{
    ...
    template<typename BaseType>
    bool AddCommonEventCallBack(BaseType* pBase, int (BaseType::* handler)(const Guid&, const int, const DataList&))
    {
        OBJECT_EVENT_FUNCTOR functor = std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
