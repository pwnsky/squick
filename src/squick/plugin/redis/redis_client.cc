...
bool RedisClient::Enable() { return m_pRedisClientSocket->IsConnect(); }

bool RedisClient::Authed() { return mbAuthed; }

bool RedisClient::Busy() { return mbBusy; }
...
bool RedisClient::SelectDB(int dbnum) { return false; }

bool RedisClient::KeepLive() { return false; }
...
bool RedisClient::AUTH(const std::string &auth) {
    ...
    if (pReply->type == REDIS_REPLY_STATUS) {
        if (std::string("OK") == std::string(pReply->str, pReply->len) || std::string("ok") == std::string(pReply->str, pReply->len)) {
            mbAuthed = true;
            return true;
        }
    }
