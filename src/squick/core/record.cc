...
int Record::GetCols() const { return mVarRecordType->GetCount(); }

int Record::GetRows() const { return mnMaxRow; }
...
DATA_TYPE Record::GetColType(const int col) const { return mVarRecordType->Type(col); }

const std::string &Record::GetColTag(const int col) const { return mVarRecordTag->String(col); }

int Record::AddRow(const int row) { return AddRow(row, *mVarRecordType); }
...
const IRecord::TRECORDVEC &Record::GetRecordVec() const { return mtRecordVec; }
