...
public:
	...
	static void CrashHandler(int sig)
	{
		...
		if (sig > 0)
		{
			if (signal(SIGILL, Exception::CrashHandler) != Exception::CrashHandler) signal(SIGILL, Exception::CrashHandler);
			if (signal(SIGABRT, Exception::CrashHandler) != Exception::CrashHandler) signal(SIGABRT, Exception::CrashHandler);
			if (signal(SIGFPE, Exception::CrashHandler) != Exception::CrashHandler) signal(SIGFPE, Exception::CrashHandler);
			if (signal(SIGSEGV, Exception::CrashHandler) != Exception::CrashHandler) signal(SIGSEGV, Exception::CrashHandler);
