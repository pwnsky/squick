...
Object::Object(Guid self, IPluginManager *pLuginManager) : IObject(self) {
...
CLASS_OBJECT_EVENT Object::GetState() { return mObjectEventState; }

void Object::SetState(const CLASS_OBJECT_EVENT eState) { mObjectEventState = eState; }

bool Object::ObjectReady() {
    CLASS_OBJECT_EVENT state = GetState();
    if (state == COE_CREATE_HASDATA || state == COE_CREATE_FINISH || state == COE_CREATE_CLIENT_FINISH) {
        return true;
    }
...
const Vector3 &Object::GetPosition() { return mPosition; }

void Object::SetPosition(const Vector3 &pos) { mPosition = pos; }
