...
template <typename T> bool List<T>::Add(const T &id) {
    // if (!Find(id))
    {
...
template <typename T> bool List<T>::Remove(const T &id) {
    if (Find(id)) {
...
template <typename T> bool List<T>::ClearAll() {
    mtObjConfigList.clear();
    return true;
}

template <typename T> bool List<T>::First(T &id) {
...
template <typename T> bool List<T>::Next(T &id) {
    if (mCurIter == mtObjConfigList.end()) {
        return false;
    }
...
template <typename T> bool List<T>::Find(const T &id) {
...
template <typename T> bool List<T>::Get(const int32_t index, T &id) {
...
template <typename T> int List<T>::Count() { return (int)(mtObjConfigList.size()); }
