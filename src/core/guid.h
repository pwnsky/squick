...
class Guid {
  public:
    ...
    Guid(const std::string &strID) { FromString(strID); }
    ...
    const INT64 GetData() const { return nData64; }

    const INT64 GetHead() const { return nHead64; }

    void SetData(const INT64 nData) { nData64 = nData; }

    void SetHead(const INT64 nData) { nHead64 = nData; }

    bool IsNull() const { return 0 == nData64 && 0 == nHead64; }

    bool operator==(const Guid &id) const { return this->nData64 == id.nData64 && this->nHead64 == id.nHead64; }
    ...
    std::string ToString() const { return lexical_cast<std::string>(nHead64) + "-" + lexical_cast<std::string>(nData64); }
