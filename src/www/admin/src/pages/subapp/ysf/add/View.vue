  ...
  <a-card :body-style="{ padding: '24px 32px' }" :bordered="false">
    <a-form :form="form">
      ...
      >
        ...
        >
          <a-radio :value="1">{{ `中医花茶` }}</a-radio>
          <a-radio :value="2">{{ `药粥` }}</a-radio>
          <a-radio :value="3">{{ `药食同源` }}</a-radio>
          <a-radio :value="4">{{ `小儿推拿` }}</a-radio>
          <a-radio :value="5">{{ `熬制中药` }}</a-radio>
        </a-radio-group>
      </a-form-item>
...
import { handleVideoAdd, handleVideoUpdate } from "@/services/subapp/ysf";
export default {
  name: "View",
  data() {
    return {
          ...
          if (this.isEdit == false) {
            ...
            handleVideoAdd(req).then((res) => {
              ...
              if (updateRes.code === 200) {
                ...
                this.$router.push("/sub_app/ysf/list");
